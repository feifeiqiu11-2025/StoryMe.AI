...
-- FIXED VERSION - Indexes created separately
-- ============================================
...
-- Indexes for character_library
CREATE INDEX idx_character_library_user_id ON character_library(user_id);
CREATE INDEX idx_character_library_name ON character_library(name);
...
-- Indexes for project_characters
CREATE INDEX idx_project_characters_project_id ON project_characters(project_id);
CREATE INDEX idx_project_characters_library_id ON project_characters(character_library_id);
...
-- Indexes for character_ratings
CREATE INDEX idx_character_ratings_image_id ON character_ratings(generated_image_id);
CREATE INDEX idx_character_ratings_character_id ON character_ratings(character_library_id);
...
-- Indexes for storybooks
CREATE INDEX idx_storybooks_user_id ON storybooks(user_id);
CREATE INDEX idx_storybooks_share_token ON storybooks(share_token);
...
-- Indexes for videos
CREATE INDEX idx_videos_storybook_id ON videos(storybook_id);
...
SELECT
  ...
  ROUND(
    ((SUM(CASE WHEN cr.rating = 'good' THEN 1 ELSE 0 END)::FLOAT /
     NULLIF(COUNT(cr.id), 0)) * 100)::NUMERIC,
    2
