...
Chinese Translation:`;
  ...
  const completion = await deepseek.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: '你是专业的儿童故事翻译专家，擅长将英文故事翻译成生动、适合儿童的中文。请注意：Connor 翻译为"康浩"，Carter 翻译为"康乐"，Lily 翻译为"莉莉"，Astronaut Lily 翻译为"宇航员莉莉"，KindleWood 保持英文不翻译。'
      },
      {
        role: 'user',
...
/**
 * Generate Chinese captions for "Astronaut Lily" story
 */
async function generateAstronautLilyChinese() {
  const projectId = 'ad7b9d41-6905-467a-a5a7-709c86ba5a81'; // Astronaut Lily story ID

  console.log('\n🚀 Starting Chinese caption generation for "Blast Off with Astronaut Lily" story...\n');
  ...
  if (error || !project) {
    console.error('❌ Error fetching Astronaut Lily story:', error);
    return;
  }
  ...
  console.log('\n' + '='.repeat(100));
  console.log('📊 TRANSLATION REVIEW - "Blast Off with Astronaut Lily" Story');
