...
Chinese Translation:`;
  ...
  const completion = await deepseek.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: '你是专业的儿童故事翻译专家，擅长将英文故事翻译成生动、适合儿童的中文。请注意：Connor 翻译为"康浩"，Carter 翻译为"康乐"，Zara 翻译为"莎拉"，Scientist Zara 翻译为"科学家莎拉"，KindleWood 保持英文不翻译。'
      },
      {
        role: 'user',
...
/**
 * Generate Chinese captions for "Scientist Zara" story
 */
async function generateScientistZaraChinese() {
  const projectId = '8933dce0-d864-4e14-91bc-7bdae9cf09d3'; // Scientist Zara story ID

  console.log('\n🚀 Starting Chinese caption generation for "Scientist Zara" story...\n');
  ...
  if (error || !project) {
    console.error('❌ Error fetching Scientist Zara story:', error);
    return;
  }
  ...
  console.log('\n' + '='.repeat(100));
  console.log('📊 TRANSLATION REVIEW - "Scientist Zara" Story');
