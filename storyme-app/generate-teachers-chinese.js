...
Chinese Translation:`;
  ...
  const completion = await deepseek.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: '你是专业的儿童故事翻译专家，擅长将英文故事翻译成生动、适合儿童的中文。请注意：Connor 翻译为"康浩"，Carter 翻译为"康乐"，Maya 翻译为"玛雅"，Teacher Maya 翻译为"玛雅老师"，KindleWood 保持英文不翻译。'
      },
      {
        role: 'user',
...
/**
 * Generate Chinese captions for "Teachers" story
 */
async function generateTeachersChinese() {
  const projectId = '5a6a676b-2e2e-4d0c-a027-7da9cbcf1346'; // Teachers story ID

  console.log('\n🚀 Starting Chinese caption generation for "Teachers" story...\n');
  ...
  if (error || !project) {
    console.error('❌ Error fetching Teachers story:', error);
    return;
  }
  ...
  console.log('\n' + '='.repeat(100));
  console.log('📊 TRANSLATION REVIEW - "Teachers" Story');
