 ...
 * Called from the Supabase Database Webhook handler on users table INSERT.
 */
...
const WELCOME_SUBJECT = 'Welcome to KindleWood — Where Kids Create, Read & Grow!';
...
export async function sendWelcomeEmail(data: WelcomeEmailData): Promise<void> {
  try {
    const { error } = await resend.emails.send({
      ...
      html: buildWelcomeHtml(data.firstName),
      replyTo: REPLY_TO,
    });
