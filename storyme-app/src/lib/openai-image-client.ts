...
/** A character/subject reference for an edit (same shape /api/edit-image sends). */
export interface OpenAIEditRef {
  name: string;
  description?: string;
  referenceImageUrl: string;
}
...
export interface OpenAIEditParams {
  /** The image being edited — used as the primary edit target (sent FIRST). */
  currentImageUrl: string;
  /** What to change, e.g. "remove the cat in the background". */
  instruction: string;
...
/**
 * Edit a scene/cover image with OpenAI gpt-image-2 — sibling to openaiGenerateScene
 * so an image generated by the batch with gpt-image-2 is also EDITED with
 * gpt-image-2 (consistent look). Sends the current image FIRST (the edit target),
 * then character references, to `images.edit` via callOpenAIImage (which handles
 * format normalization + multi-image→single fallback; the current image stays the
 * target even on fallback). Returns a PNG data URL in the same shape the Gemini
 * edit returns, so /api/edit-image's upload path is unchanged.
 */
export async function openaiEditScene({
  currentImageUrl,
  instruction,
  sceneDescription,
  styleVariant,
  ...
  const startTime = Date.now();
  const variant = styleVariant || 'classic';
  const styleLine = SCENE_STYLE_LINES[variant] || SCENE_STYLE_LINES.classic;

  // The current image is the edit target and MUST be first.
  const currentBuffer = await downloadImageAsBuffer(currentImageUrl);

  // Character/subject references (best-effort — a failed fetch drops just that ref).
  const refs = (characterReferences || []).filter((c) => c.referenceImageUrl && c.referenceImageUrl.trim());
  const fetched = (
    await Promise.all(
      refs.map(async (c) => {
        try {
          ...
          console.warn(`[openai-edit] reference fetch failed for "${c.name}":`, err instanceof Error ? err.message : err);
          return null;
        }
      })
    )
  ).filter((x): x is { ref: OpenAIEditRef; buf: Buffer } => x !== null);

  // Optional manual reference (base64 → buffer).
  let manualBuf: Buffer | null = null;
  if (manualReferenceImageBase64) {
    try {
      const b64 = manualReferenceImageBase64.includes(',')
        ? manualReferenceImageBase64.split(',')[1]
        : manualReferenceImageBase64;
      manualBuf = Buffer.from(b64, 'base64');
    } catch {
      manualBuf = null;
    }
  }

  const buffers: Buffer[] = [currentBuffer, ...fetched.map((f) => f.buf), ...(manualBuf ? [manualBuf] : [])];

  // Edit prompt: instruction first, then labeled references, then style + safety.
  const lines: string[] = [
    'Edit the FIRST attached image as instructed below. Keep the overall composition, characters, and setting the same except for the requested change, and match the existing art style exactly.',
    '',
