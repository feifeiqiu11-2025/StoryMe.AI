 ...
 * - Tag filter: ?tag=Some+Tag+Name (filters by gallery tag)
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (featured) {
      ...
      const featuredResponse = NextResponse.json({ characters: characters || [] });
      featuredResponse.headers.set('Cache-Control', 'public, s-maxage=60, stale-while-revalidate=300');
    ...
    let query = supabase
      .from('character_library')
      .select('id, name, reference_image_url, animated_preview_url, created_at, designer_name, designer_age', { count: 'exact' })
      .eq('is_public', true)
      .not('reference_image_url', 'is', null)
      .not('animated_preview_url', 'is', null);
    ...
    paginatedResponse.headers.set('Cache-Control', 'public, s-maxage=60, stale-while-revalidate=300');
