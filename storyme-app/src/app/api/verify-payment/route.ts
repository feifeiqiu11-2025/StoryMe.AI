...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isPaymentComplete && userData && userData.subscription_tier !== subscriptionTier) {
      ...
      // Privileged columns (subscription_tier, trial_status, stories_limit, etc.)
      // are blocked by the RLS guard for the authenticated role. Use the
      // service-role client for this write.
